...
describe('SearchController', () => {
  ...
  describe('search/{service}', () => {
    ...
    it('should return a search response with 3 titles', async () => {
      jest.spyOn(catalogSearchService, 'getSearchResults').mockImplementation(() => of(result));

      await expect(lastValueFrom(searchController.search('cats', 'catalog'))).resolves.toEqual(result);
    });
  });
});
